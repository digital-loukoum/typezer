	...
	private parseCommentsIntoDefinition(
		...
		jsdocs.forEach(doc => {
			...
			if (name.startsWith("TJS-")) {
				...
				let match: string[] | RegExpExecArray | null = new RegExp(REGEX_TJS_JSDOC).exec(
					originalText
				)
				if (match) {
					name = match[1]
		...
		const props = this.tc.getPropertiesOfType(clazzType).filter(prop => {
			...
			return !(
				...
				decls.filter(decl => {
					...
					return (
						mods &&
						mods.filter(mod => mod.kind === ts.SyntaxKind.PrivateKeyword).length > 0
					)
				}).length > 0
			)
		})
		...
		return new Promise((resolve, reject) => {
			...
			fs.mkdir(path.dirname(args.out), { recursive: true }, function (mkErr: Error) {
				if (mkErr) {
					return reject(
						new Error(
							"Unable to create parent directory for output file: " + mkErr.message
						)
					)
				}
